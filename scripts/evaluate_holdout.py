...
# Quality adjusted PCS contribution (PCS * p_hat)
df['q_PCS'] = df['PCS'] * df['p_hat']

# Split train and holdout
# Train: Nov 2023 - Feb 2024 (months 11, 12, 1, 2)
# Holdout: April 2024 (month 4)
train_df = df[df['month'].isin([11, 12, 1, 2])].copy()
holdout_df = df[df['month'] == 4].copy()

print(f"Train records: {len(train_df):,}")
print(f"April holdout records: {len(holdout_df):,}")

# ─────────────────────────────────────────────────────────────────────────────
# RE-FIT POISSON MODEL ON TRAIN DATA
# ─────────────────────────────────────────────────────────────────────────────
print("Fitting corrected Poisson GLM on TRAIN data...")
...
print(f"hour coef: {hour_coef:.4f}")
print(f"day_type coef: {day_type_coef:.4f}")
print(f"log(Z_prev + 1) coef: {log_Z_coef:.4f}\n")

# ─────────────────────────────────────────────────────────────────────────────
# COMPUTE COMPOSITE SCORE (EPS_v1)
# ─────────────────────────────────────────────────────────────────────────────
...
# calculate Persistence Index (PI_i)
monthly_top50 = {}
for m in [12, 1, 2]:
...
# Setup combinations for weekly volume prediction
...
# EVALUATION PIPELINE
# ─────────────────────────────────────────────────────────────────────────────
total_april_violations = len(holdout_df)
print(f"Total April violations: {total_april_violations:,}")

# Top 20 April cells by actual count (for stability check)
...
# Baseline Top 20 (raw counts in training)
baseline_top20 = Zi.nlargest(20, 'raw_count_train')['grid_id'].tolist()
actual_in_baseline = holdout_df[holdout_df['grid_id'].isin(baseline_top20)].shape[0]
pct_baseline = actual_in_baseline / total_april_violations * 100
...
# Case A: Z_prev = training average monthly PCS
...
# Case B: Z_prev = actual March PCS sum
...
for label, combos in [("Case A (Training Average Monthly PCS)", combos_a), ("Case B (Actual March PCS)", combos_b)]:
    print(f"\n--- EPS_v1 RESULTS ({label}) ---")
    
    # Predict lambdas (month-frozen, setting month effect to 0.0)
    combos['log_lambda'] = (
        Intercept +
    ...
    # Updated EPS formula (Remove double-counting of Z_i)
    eval_df['EPS_v1'] = eval_df['PI'] * eval_df['pred_weekly_volume']
    
    # Get Top 20 target cells by EPS_v1
    eps_top20_df = eval_df.nlargest(20, 'EPS_v1').copy()
    eps_top20 = eps_top20_df['grid_id'].tolist()
    actual_in_eps = holdout_df[holdout_df['grid_id'].isin(eps_top20)].shape[0]
    pct_eps = actual_in_eps / total_april_violations * 100
    stability_eps = sum(1 for g in eps_top20 if g in top20_actual)
    
    # Get actual count in April for each top cell
    actuals = []
    for g in eps_top20:
        act = holdout_df[holdout_df['grid_id'] == g].shape[0]
        actuals.append(act)
    eps_top20_df['april_actual'] = actuals
    ...
    print("\nTop 10 Grid Cells:")
    print(eps_top20_df.head(10)[['grid_id', 'PI', 'pred_weekly_volume', 'EPS_v1', 'april_actual', 'in_top20_actual']].to_string(index=False))
